...
// INT16 sYStart = 0;
 ...
 INT16 sYAdd = 0;
	...
	usColor=Get16BPPColor( FROMRGB( GlowColorsA[iColorNum].ubRed, GlowColorsA[iColorNum].ubGreen, GlowColorsA[iColorNum].ubBlue ) );
	pDestBuf = LockVideoSurface( FRAME_BUFFER, &uiDestPitchBYTES );
	SetClippingRegionAndImageWidth( uiDestPitchBYTES, 0, 0, SCREEN_WIDTH_1024, SCREEN_HEIGHT_768);
...
//		fDisabledMapBorder = FALSE;
		...
		if(fPreLoadedMapGraphics == FALSE )
		{
			...
			if (iResolution == 0)
			{
				...
				MAP_BORDER_X_OFFSET	= (((SCREEN_WIDTH_1024 - 261) - 380) / 2);	// 380: Width of the MBS.sti file
				MAP_BORDER_Y_OFFSET = (((SCREEN_HEIGHT_768 - 121) - 360) / 2);  // 360: Height of the MBS.sti file
				...
				// Map Level string
				MAP_LEVEL_STRING_X	= (SCREEN_WIDTH_1024 - 208);
				MAP_LEVEL_STRING_Y	= (SCREEN_HEIGHT_768 - 175);
			...
			else if (iResolution == 1)
			{
				...
				MAP_BORDER_X_OFFSET	= (((SCREEN_WIDTH_1024 - 261) - 540) / 2);
				MAP_BORDER_Y_OFFSET = (((SCREEN_HEIGHT_768 - 121) - 480) / 2);
				...
				// Map Level string
				MAP_LEVEL_STRING_X	= (SCREEN_WIDTH_1024 - 208 - 80);
				MAP_LEVEL_STRING_Y	= (SCREEN_HEIGHT_768 - 175);
			...
			else if (iResolution == 2)
			{
				...
				MAP_BORDER_X_OFFSET	= (((SCREEN_WIDTH_1024 - 261) - 764) / 2);
				MAP_BORDER_Y_OFFSET = (((SCREEN_HEIGHT_768 - 121) - 648) / 2);
				...
				// Map Level string
				MAP_LEVEL_STRING_X	= (SCREEN_WIDTH_1024 - 208 - 187);
				MAP_LEVEL_STRING_Y	= (SCREEN_HEIGHT_768 - 175);
...
//			fShowMapScreenHelpText = TRUE;
		...
		MSYS_DefineRegion( &gMapScreenMaskRegion, 0, 0, SCREEN_WIDTH_1024, SCREEN_HEIGHT_768, MSYS_PRIORITY_LOW,
...
//	RenderMapBorderCorner( );
	...
	if( !gfWorldLoaded )
	{
		...
		if( GetJA2Clock() % 1000 < 500 )
			...
			SetFontForeground( FONT_RED );
		mprintf( SCREEN_WIDTH_1024 - 110, 2, L"TESTVERSION MSG" );
		if( GetJA2Clock() % 1000 < 500 )
			SetFontForeground( FONT_DKYELLOW );
		else
			SetFontForeground( FONT_YELLOW );
		mprintf( SCREEN_WIDTH_1024 - 110, 12, L"NO WORLD LOADED" );
		InvalidateRegion( SCREEN_WIDTH_1024 - 110, 2, SCREEN_WIDTH_1024, 23 );
	}
	#endif
...
*/
	...
	if ( gfLoadPending == 1 )
	{
		...
		//Shadow area
		ShadowVideoSurfaceRect( FRAME_BUFFER, 0, 0, SCREEN_WIDTH_1024, SCREEN_HEIGHT_768 );
		InvalidateScreen( );
	}
...
void PopupText( STR16 pFontString, ...	)
{
	...
	FindFontCenterCoordinates( 0, 0, SCREEN_WIDTH_1024, INTERFACE_START_Y, PopupString, LARGEFONT1, &sX, &sY );
	...
	else if (iReason & MSYS_CALLBACK_REASON_RBUTTON_UP && fRightDown )
	{
		...
		if ( ( pSoldier->inv[ uiHandPos ].ubNumberOfObjects > 1 ) && ( isLimit > 0 ) )
		{
			if ( !InItemStackPopup( ) )
			{
				...
				InitItemStackPopup( pSoldier, (UINT8)uiHandPos, 0, INV_REGION_Y, 261, ( SCREEN_HEIGHT_768 - PLAYER_INFO_Y ) );
				fTeamPanelDirty=TRUE;
				fInterfacePanelDirty = DIRTYLEVEL2;
			}
		}
...
void RenderAttributeStringsForUpperLeftHandCorner( UINT32 uiBufferToRenderTo )
{
	...
	SetFontForeground( CHAR_TITLE_FONT_COLOR );
	SetFontBackground( FONT_BLACK );
	SetFontDestBuffer( uiBufferToRenderTo, 0, 0, SCREEN_WIDTH_1024, SCREEN_HEIGHT_768, FALSE );
...
void RenderMapRegionBackground( void )
{
	...
	RestoreExternBackgroundRect( 261, 0, SCREEN_WIDTH_1024 - 261, SCREEN_HEIGHT_768 - 121 );
...
void RenderTeamRegionBackground( void )
{
	...
	if(fShowInventoryFlag == TRUE && UsingNewInventorySystem() == false){
		RestoreExternBackgroundRect( 0, 107, 261, SCREEN_HEIGHT_768 - 106 - 121 );
	}
	else if(fShowInventoryFlag == TRUE && iResolution == 0){
		RestoreExternBackgroundRect( 0, 107, 261, SCREEN_HEIGHT_768 - 107 );
	}
	else if(fShowInventoryFlag == TRUE && iResolution == 1){
		RestoreExternBackgroundRect( 0, 107, 261, SCREEN_HEIGHT_768 - 107 );
	}
	else if(fShowInventoryFlag == FALSE || iResolution == 2){
		RestoreExternBackgroundRect( 0, 107, 261, SCREEN_HEIGHT_768 - 107 - 121 );
	}
	MapscreenMarkButtonsDirty();
	...
	if( fNewMailFlag )
	{
		if( GetJA2Clock() % 1000 < 667 )
		{
			...
			{ //button is down, so offset the icon
				...
				BltVideoObjectFromIndex( FRAME_BUFFER, guiNewMailIcons, 1, (SCREEN_WIDTH_1024 - 175), (SCREEN_HEIGHT_768 - 62), VO_BLT_SRCTRANSPARENCY, NULL );
				//InvalidateRegion( 465, 418, 480, 428 );
				InvalidateRegion( (SCREEN_WIDTH_1024 - 175), (SCREEN_HEIGHT_768 - 62), (SCREEN_WIDTH_1024 - 160), (SCREEN_HEIGHT_768 - 52 ));
			}
			else
			{	//button is up, so draw the icon normally
				...
				BltVideoObjectFromIndex( FRAME_BUFFER, guiNewMailIcons, 0, (SCREEN_WIDTH_1024 - 176), (SCREEN_HEIGHT_768 - 63), VO_BLT_SRCTRANSPARENCY, NULL );
				if( !(ButtonList[ guiMapBottomExitButtons[ MAP_EXIT_TO_LAPTOP ] ]->uiFlags & BUTTON_ENABLED ) )
				{
					UINT32 uiDestPitchBYTES;
					UINT8	*pDestBuf;
					SGPRect area = { (SCREEN_WIDTH_1024 - 177), (SCREEN_HEIGHT_768 - 63), (SCREEN_WIDTH_1024 - 163), (SCREEN_HEIGHT_768 - 55) };

				pDestBuf = LockVideoSurface( FRAME_BUFFER, &uiDestPitchBYTES );
					Blt16BPPBufferHatchRect( (UINT16*)pDestBuf, uiDestPitchBYTES, &area );
					UnLockVideoSurface( FRAME_BUFFER );
				}
				InvalidateRegion( (SCREEN_WIDTH_1024 - 177), (SCREEN_HEIGHT_768 - 63), (SCREEN_WIDTH_1024 - 159), (SCREEN_HEIGHT_768 - 50) );
