...
void GoIntoOverheadMap( )
	...
	else if (iResolution == 2)
	{
		...
		if (iResolution_BIG == 3) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1280x768.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 4) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1280x800.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 5) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1280x960.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 6) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1280x1024.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 7) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1360x768.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 8) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1366x768.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 9) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1400x1050.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 10) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1600x900.sti", VObjectDesc.ImageFile);
		} else if (iResolution_BIG == 11) {
			FilenameForBPP("INTERFACE\\MAP_BORD_1680x1050.sti", VObjectDesc.ImageFile);
...
void RenderOverheadMap( INT16 sStartPointX_M, INT16 sStartPointY_M, INT16 sStartPointX_S, INT16 sStartPointY_S, INT16 sEndXS, INT16 sEndYS, BOOLEAN fFromMapUtility )
{
	...
	if ( gfOverheadMapDirty )
	{
		...
		// Nur Karte und position der geb鋟de
		do
		{
			fEndRenderRow = FALSE;
